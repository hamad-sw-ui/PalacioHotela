...
- `/devis` : panier multi-prestations en JSON `selected_items` (hébergement, restauration, salles, services), demande enregistrée sous `DEV-YYYY-NNNNN` et accusé de réception PDF. Le backoffice renseigne le montant **global TTC en XAF** et la date de validité, puis envoie le PDF de devis. Le lien privé du client permet de suivre, télécharger et, si l'option est activée, accepter le devis. Aucun devis n'est converti automatiquement en facture.
